...
@section('content')
    <form action="" method="post">
        ...
        <div class="form-group has-feedback {{ $errors->has('name') ? 'has-error' : '' }}">
            <input type="text" name="typeof_name" class="form-control" value="{{$class -> typeof_name}}"
                   placeholder="类型名称">
        </div>
        <div class="form-group has-feedback {{ $errors->has('password_confirmation') ? 'has-error' : '' }}">
            <input type="text" class="form-control" value="{{$class -> typeof_num}}" name="typeof_num">
        </div>
        <div class="form-group has-feedback {{ $errors->has('password_confirmation') ? 'has-error' : '' }}"">
            <label>
                <input type="radio" name="typeof_status" value="1" class="minimal" @if($class -> typeof_status == 1) checked @endif>
            </label>
            <label>
                <input type="radio" class="minimal" disabled>正常
            </label>
            <label style="margin-left: 15px;">
                <input type="radio" name="typeof_status"  @if($class -> typeof_status == 0) checked @endif class="minimal">
            </label>
            <label>
