...
@section('content')
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group has-feedback {{ $errors->has('name') ? 'has-error' : '' }}" style="position: relative !important;">
            <input type="text" name="type_name" class="form-control" value="{{ old('name') }}"
                   placeholder="分类名称">
        ...
        <div class="form-group has-feedback {{ $errors->has('password_confirmation') ? 'has-error' : '' }}">
            <label class="radio-inline">
                <input type="radio" name="type_status" value="0">宣传页
            </label>
            <label class="radio-inline">
                <input type="radio" checked name="type_status" value="1">列表
            </label>
        </div>
        <div class="form-group has-feedback {{ $errors->has('password_confirmation') ? 'has-error' : '' }}">
            <div class="form-group">
                <select name="parent_id" id="" class="form-control">
                    <option value="0">新顶级分类</option>
                    @foreach($types as $k => $type)
                        <option value="{{$type -> type_id}}">{{str_repeat('--',$type -> level) .$type -> type_name}}</option>
                        @if(!empty($type -> son))
                            @foreach($type['son'] as $k => $tyson)
                            <option value="{{$tyson -> type_id}}">{{str_repeat('--',$tyson -> level) .$tyson -> type_name}}</option>
                            @endforeach
                        @endif
                    @endforeach
                </select>
