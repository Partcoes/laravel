                        ...
                        @if($buttons)
                            @foreach($buttons as $k => $button)
                                @if($button -> button_name == '删除')
                                    <button href="javascript:void(0)" alerturl="{{url($button -> button_url)}}?id={{$user -> admin_id}}" class="btn {{$button -> class}}">{{$button -> button_name}}</button>
                                @else
                                    <a href="{{url($button -> button_url)}}?id={{$user-> admin_id}}" alerturl="{{url($button -> button_url)}}?id={{$user -> admin_id}}" class="btn {{$button -> class}}">{{$button -> button_name}}</a>
                                @endif
                            @endforeach
                        @endif
                    </th>
